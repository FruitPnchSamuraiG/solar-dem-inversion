...
def dumpSynthesis(target, name, AIACube, DEMCube, R, wavelengths):
    ...
    for i in range(AIACube.shape[0]):
        ...
        k = ~np.isnan(x)
        low = np.nanmin(np.log10(x[k]))
        clip = np.maximum(np.nanmax(np.log10(x[k])), np.nanmax(np.log10(y[k])))
        if clip <= low:
            clip = low + 1.0

        plt.figure(figsize=(4,4))
        plt.hexbin(x[k], y[k], xscale='log', yscale='log', extent=[low,clip,low,clip], bins='log', gridsize=200)
        plt.plot([10**low, 10**clip],[10**low, 10**clip], c='k', linestyle='--')
        plt.axis('square')
