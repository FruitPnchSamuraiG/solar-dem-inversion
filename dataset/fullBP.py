...
def invertDEMCube(AIACube, AIAErrors, RData, args):
    ...
    bp_elapsed = toc - tic
    total_pixels = H * W
    n_cores = doParallel if doParallel > 0 else multiprocessing.cpu_count()
    print("BP took %.2f" % bp_elapsed)
    print(f"SPEED (wall-clock, {n_cores} cores): {total_pixels:,} px in {bp_elapsed:.2f}s  |  {total_pixels / bp_elapsed:,.0f} px/s  |  {bp_elapsed / total_pixels * 1e6:.3f} us/px")
    print(f"SPEED (single-core equivalent):      {bp_elapsed * n_cores / total_pixels * 1e6:.3f} us/px  |  {total_pixels / (bp_elapsed * n_cores):,.0f} px/s per core")
...
def parseArgs():
    ...
    parser.add_argument("--corr_table", default=os.path.join(os.path.dirname(os.path.abspath(__file__)), "aia_corr.csv"), help="correction table to use for degradation factors (csv is better)")
    ...
    parser.add_argument("--deconvolve", default="none", type=str, choices=["none", "aiapy", "hofmeister"],
                        help="deconvolution method: none, aiapy (built-in PSF), or hofmeister (BID GPU)")
...
if __name__ == "__main__":
    ...
    elif args.deconvolve == "hofmeister":
        sys.path.insert(0, os.path.join(os.path.dirname(__file__), 'hofmeister_psf'))
        from deconvolve_image import deconvolve_bid
        PSF_DIR = os.path.join(os.path.dirname(__file__), 'hofmeister_psf')
        AIAMaps = [update_pointing(m, pointing_table=pointing_table) for m in AIAMaps]
        AIAMapsN = []
        for i in range(len(AIAMaps)):
            wl = wavelengths[i]
            print(f"Hofmeister deconvolution: loading PSF for {wl}A")
            psf_data = fits.getdata(os.path.join(PSF_DIR, f'psf_aia_{wl}.fits')).astype(np.float32)
            print(f"Hofmeister deconvolution: deconvolving channel {i} ({wl}A)")
            decon_data = deconvolve_bid(AIAMaps[i].data.astype(np.float32), psf_data, use_gpu=False, large_psf=True)
            AIAMapsN.append(sunpy.map.Map(decon_data, AIAMaps[i].meta))
        AIAMaps = AIAMapsN
        AIAMaps = [register(m) for m in AIAMaps]
    else:
